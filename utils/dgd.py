...
def DGD_cst_stepsize(x_m, agents_x, W, agents_y, alpha_star, n_iter = 100, sigma = 0.5, nu = 1.0, eta = 0.01):
    m = len(x_m)
...
def DGD_vanishing_stepsize(x_m, agents_x, W, agents_y, alpha_star, n_iter = 100, sigma = 0.5, nu = 1.0):
    m = len(x_m)
    n_agents = len(agents_x)
    ...
    for t in range(n_iter):
        ...
        # Gradient step
        alpha = [
            new_alpha_matrix[i] - (1/(t+1)**2) * local_gradient(new_alpha_matrix[i], agents_x[i], agents_y[i], x_m)
            for i in range(n_agents)
        ]
        gap = [np.linalg.norm(alpha[i] - alpha_star) for i in range(n_agents)]
        gaps.append(gap)
